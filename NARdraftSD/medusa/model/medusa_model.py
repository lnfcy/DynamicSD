...
class MedusaModel():
    @staticmethod
    def _identify_model_type(model_name_or_path):
        """从模型名称识别模型类型"""
        model_name = model_name_or_path.lower()
        if any(name in model_name for name in ['llama', 'vicuna', 'alpaca']):
            return "llama"
        elif any(name in model_name for name in ['mistral', 'mixtral']):
            return "mistral"
        else:
            # 如果无法从名称判断，尝试加载配置文件
            try:
                ...
                raise ValueError(
                    f"Cannot identify model type from name '{model_name_or_path}'. "
                    "Model name should contain 'llama', 'vicuna', 'alpaca', 'mistral' or 'mixtral', "
                    "or the model config should specify model_type."
                )
    ...
    def from_pretrained(
        ...
        **kwargs,
        ...
        try:
            ...
            base_model_config = AutoConfig.from_pretrained(config.base_model_name_or_path)
            # 使用模型名称识别替代配置文件识别
            config.model_type = cls._identify_model_type(pretrained_model_name_or_path)
